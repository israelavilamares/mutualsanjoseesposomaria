...
namespace mutualsanjoseesposomaria
{
    public partial class pagosunicos : Form
    {
        ...
        private void pagosunicos_Load(object sender, EventArgs e)
        {
            ...
            string query = "SELECT socios.id as numero_de_socio, socios.nombre,apellido,sexo,fecha_ingreso,ciudad,colonia,calle,telefono,edad,fecha_naciento,tipo_s.nombre as tipo_pago,socios.observaciones FROM socios,tipo_s WHERE socios.tipo_p = tipo_s.id AND tipo_s.nombre = 'PAGO UNICO'";
        ...
        private void busca_Click(object sender, EventArgs e)
        {
          //  int search = int.Parse(busquedatxt.Text);
            ...
            DataTable datatable = new DataTable();
            string query = "SELECT socios.id as numero_de_socio, socios.nombre,apellido,sexo,fecha_ingreso,ciudad,colonia,calle,telefono,edad,tipo_s.nombre as tipo_pago FROM socios JOIN tipo_s ON socios.tipo_p = tipo_s.id " +
        ...
        private void apellidotxb_TextChanged(object sender, EventArgs e)
        {
            DataTable datatable = new DataTable();
            string query = "SELECT socios.id as numero_de_socio, socios.nombre,apellido,sexo,fecha_ingreso,ciudad,colonia,calle,telefono,edad,tipo_s.nombre as tipo_pago FROM socios JOIN tipo_s ON socios.tipo_p = tipo_s.id " +
                "WHERE socios.apellido LIKE '%" + apellidotxb.Text + "%' AND tipo_s.id = 3";
